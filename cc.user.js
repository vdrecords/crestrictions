...
// @version      0.12.6
// @description  chess.com/lichess.org: задачи + Blitz≥3+0/Rapid/Classical. v0.12: Bullet-награда — окно 10–60 мин в конце расписания при solved≥400 (динамически растёт +10 мин/+100 задач до cap 60). UI прозрачный для ребёнка (4 состояния), работает в любой день недели, master toggle BULLET_REWARD_ENABLED. v0.12.2: компактный 2-строчный layout. v0.12.3: BULLET_REWARD_FORCE_OPEN_DATES — особые дни, 1 час Bullet гарантирован независимо от решённых задач. v0.12.4: критфикс — доска не скрывается на странице Bullet-партии при открытом окне (textHasAllowedType добавляет 'Пуля'/'Bullet'). v0.12.5: вт/чт вечернее окно сдвинуто на 17:00–20:00 (было 18:00) — +1 час игры в эти дни. v0.12.6: пятница — вечернее окно с 15:30 (было 18:00); override 2026-05-15 — утреннее окно до 15:30 (непрерывный коридор 09:00–20:00 сегодня).
// @author       vdrecords
...
(function () {
    ...
    const SCHEDULE_WEEKLY = {
        ...
        2: [['09:00', '12:00'], ['17:00', '20:00']], // Вторник
        3: [['09:00', '12:00'], ['18:00', '20:00']], // Среда
        4: [['09:00', '12:00'], ['17:00', '20:00']], // Четверг
        5: [['09:00', '12:00'], ['15:30', '20:00']], // Пятница
    ...
    const SCHEDULE_OVERRIDES = {
        ...
        '2026-05-15': { // Разовое продление утреннего окна до 15:30 (стыкуется с новым пятничным вечерним окном — непрерывный коридор 09:00–20:00)
            patch: [{ index: 0, to: '15:30' }]
        }
    };
