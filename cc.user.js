...
// @grant        unsafeWindow
// @connect      cfg.allcantrip.ru
// @connect      mrwower.fvds.ru
...
(function () {
    ...
    // v0.19: список ПУСТ и должен таким оставаться. Разовые продления выдаёт бот
    // @chessday_control_bot, и он пишет их в удалённый конфиг с сегодняшней датой,
    // затирая вчерашнюю. Здесь они больше не накапливаются.
    // Прежние даты (16.11.25, 23.12.25, 09.05.26, 20.08.26) удалены 12.09.2026 —
    // все в прошлом, ни на что не влияли, но мешали читать заводское состояние.
    // Руками сюда вписывать что-либо стоит только если бот недоступен.
    ...
    // v0.19: пусто по той же причине, что и SCHEDULE_OVERRIDES — разовую норму
    // на сегодня выдаёт бот.
    ...
    const LICHESS_DISABLED_DATES = []; // v0.19: разовые даты выдаёт бот
    ...
    const LICHESS_FULL_UNLOCK_DATES = []; // v0.19: свободные дни выдаёт бот
    ...
    // ─── 5.4 YOUTUBE НА КОНКРЕТНЫЙ ДЕНЬ (v0.24) ──────────────────────────────
    // youtube.com лежит в blockedHosts и оттуда НЕ вынимается: список доменов —
    // ядро запрета. Разрешение сделано отдельным слоем: дата (YYYY-MM-DD), на
    // которую ютуб открыт, проверяется раньше блок-листа и только на хостах из
    // YOUTUBE_HOSTS. Любой другой домен идёт прежним путём, ошибиться и открыть
    // заодно весь интернет здесь нечем.
    //
    // Расписание блокировки компьютера действует и в такой день: вне окна экран
    // закрыт, как обычно (тот же принцип, что у свободного lichess, v0.17.2).
    const YOUTUBE_UNLOCK_DATES = []; // v0.24: разовые дни выдаёт бот

    // Когда действует разрешение внутри дня:
    //   'schedule' — только внутри окон расписания дня (рекомендуемый режим);
    //   'always'   — весь календарный день (вне окон всё равно закроет timeBlocker).
    const YOUTUBE_UNLOCK_MODE = 'schedule';

    // Персональные часы на дату (перебивают MODE).
    // Формат: 'YYYY-MM-DD': [['HH:MM', 'HH:MM'], ...]
    const YOUTUBE_UNLOCK_WINDOWS = {
        // '2026-09-20': [['16:00', '17:00']]
    };

    // true — разрешение включается только ПОСЛЕ выполнения дневной нормы задач.
    // Пока норма не сделана, ютуб закрыт блок-экраном, как в обычный день.
    const YOUTUBE_UNLOCK_AFTER_TASK_TARGET = false;

    // Хосты, на которые распространяется разрешение. Ядро запрета: удалённо НЕ
    // управляется, правится только здесь. Поддомены (www., m., music.) попадают
    // сюда сами — hostMatches сверяет и `host === base`, и `*.base`.
    const YOUTUBE_HOSTS = ['youtube.com', 'youtu.be'];

    // v0.24.3. Спутники: домены, без которых до ютуба не добраться привычным
    // способом. Адрес целиком ребёнок не набирает — пишет слово в адресной
    // строке, оно уходит в поиск, и блок-экран выпадает на поиске (журнал 21.09:
    // за всё утро ни одного обращения к youtube.com, зато три к google.com).
    //
    // 🔴 Список сверяется ТОЛЬКО точным совпадением хоста, в отличие от
    // YOUTUBE_HOSTS. Запись 'google.com' в обычной проверке открыла бы весь
    // `*.google.com` — почту, диск и chromewebstore.google.com, то есть
    // установку расширений, которой родительский контроль и обходят.
    // Ядро запрета: удалённо НЕ управляется.
    const YOUTUBE_COMPANION_HOSTS = ['google.com', 'www.google.com'];
    ...
    const BULLET_REWARD_FORCE_OPEN_DATES = []; // v0.19: особые дни выдаёт бот
    ...
    const LOCAL_CONFIG = {
        ...
        remoteConfig: {
            // v0.19 (12.09.2026): удалённое управление ВКЛЮЧЕНО. Файл на сервере
            // редактирует Telegram-бот @chessday_control_bot, скрипт опрашивает его
            // раз в минуту и применяет изменения на лету.
            //
            // Почему это безопасно без HMAC-подписи (вопрос из v0.18):
            //   • файл отдаётся только на чтение по HTTPS — подменить его по дороге
            //     нельзя без корневого сертификата, а для этого нужны права
            //     администратора, которых у ребёнка нет;
            //   • записать в файл можно лишь по ssh с родительского ключа;
            //   • секрет для подписи всё равно пришлось бы держать в тексте скрипта,
            //     а скрипт лежит в публичном репозитории — подпись была бы бутафорией.
            // Остаётся один реальный сценарий: пропала сеть. Его закрывает cacheMaxAgeMs.
            enabled: true,
            // Основное имя — свой поддомен: у него собственный лимит на выпуск
            // сертификатов. Служебное имя хостера mrwower.fvds.ru указывает на тот же
            // сервер и остаётся в @connect как запасное, но упирается в чужой лимит
            // Let's Encrypt по домену fvds.ru, поэтому основным быть не может.
            url: 'https://cfg.allcantrip.ru/tm/chess-control-config.json',
            fetchIntervalMs: 60000,
            requestTimeoutMs: 5000,
            // Срок годности кэша. Кэш хранит ПОСЛАБЛЕНИЯ, выданные родителем, и они не
            // должны переживать долгую потерю связи с сервером: иначе «сегодня играем
            // до 21:00», выданное однажды, работало бы вечно при выключенной сети.
            // Протух — стираем, дальше действуют локальные настройки этого скрипта.
            ...
            allowedPaths: {
                ...
                'lichess.org': {
                    ...
                    blockRegex: [
                        ...
                        // v0.23: раздел команд. Из block список '/team' убран сознательно:
                        // там сравнение по префиксу, и оно закрывало заодно саму страницу
                        // команды, на которую ребёнку нужно попасть ради швейцарок.
                        // Эти два правила закрывают ровно то, ради чего раздел и закрывали.
                        // Формально их хватило бы и без явной записи — всё, чего нет в allow,
                        // и так падает в отказ, — но писать намерение явно честнее: правило
                        // переживёт будущее расширение allow-списка, а молчаливый отказ нет.
                        '^/team/?$',          // каталог команд: поиск, чужие команды
                        '^/team/[^/]+/.+'     // любой подраздел команды: форум, рассылка, участники
                    ],
                    allow: [
                    ...
                    allowRegex: [
                        ...
                        // v0.23: САМА страница команды и ничего вглубь неё. Нужна, чтобы
                        // вступить в команду и оттуда попасть в её швейцарский турнир.
                        '^/team/[^/]+$',
        ...
        // v0.24: разовое разрешение ютуба. hosts — ядро запрета, удалённо НЕ
        // управляется; бот выдаёт только дату, окно, режим и признак «после нормы».
        youtube: {
            hosts: YOUTUBE_HOSTS,
            companionHosts: YOUTUBE_COMPANION_HOSTS,
            unlockDates: YOUTUBE_UNLOCK_DATES,
            unlockMode: YOUTUBE_UNLOCK_MODE,
            unlockWindows: YOUTUBE_UNLOCK_WINDOWS,
            afterTaskTarget: YOUTUBE_UNLOCK_AFTER_TASK_TARGET
        },
    ...
    const REMOTE_CONFIG_PATHS = [
        ...
        // v0.19: минимальный контроль времени. Правится вместе с производным
        // lichess.minBaseMinutes — см. syncDerivedConfig().
        'chessCom.minBaseTimeSeconds',
        // v0.19: награда Bullet. До этой версии в списке не было ни одного её поля,
        // то есть «дать час пули на сегодня» удалённо не работало вообще.
        'bulletReward.enabled',
        'bulletReward.threshold',
        'bulletReward.minutesAtThreshold',
        'bulletReward.extraMinutesPerStep',
        'bulletReward.stepTaskCount',
        'bulletReward.capMinutes',
        'bulletReward.minBulletSeconds',
        'bulletReward.disabledDates',
        'bulletReward.forceOpenDates',
        ...
        // v0.24: разовое разрешение ютуба. Хосты (youtube.hosts) в список
        // сознательно НЕ попали — это ядро запрета, см. комментарий ниже.
        'youtube.unlockDates',
        'youtube.unlockWindows',
        'youtube.afterTaskTarget',
    ...
    //   urlBlocker.*            — разрешённые домены и разделы. Ядро default-deny:
    //                             ошибка здесь открывает переписку;
    //   sendGuard.*             — предохранитель отправки, последняя линия;
    //   youtube.hosts           — на какие домены распространяется разрешение
    //                             ютуба. Подменить их удалённо — открыть любой
    //                             сайт одной строкой в файле;
    //   youtube.unlockMode      — 'schedule' против 'always'. Бот им не управляет
    //                             и управлять не должен: пока действует расписание,
    //                             режимы неразличимы, а список обязан совпадать
    //                             с тем, чем бот реально управляет;
    //   lichess.blockedTrainingPaths, lichess.allowedGameTypes,
    //   chessCom.blocked*       — фильтры содержимого, меняются раз в год;
    //   timeBlocker.warningMinutes, pollIntervalMs, tracker.processedRaceKeepDays,
    //   tracker.chessComPuzzlesRoot, messageControl.tasksPerMessage — техника.
    // Правило простое: в списке ровно то, чем управляет бот, и ни поля больше.
    ...
    const RUNTIME = {
        ...
        // v0.24: состояние «ютуб открыт ПРЯМО СЕЙЧАС», та же механика.
        youtube: {
            active: null
        },
        autoHide: {
    ...
    // ═════════════════════════════════════════════════════════════════════════
    // YOUTUBE НА КОНКРЕТНЫЙ ДЕНЬ (v0.24)
    //
    // Устроено как свободный lichess, но проще: разрешение по дате + окно внутри
    // дня. Разница в том, где стоит проверка — не вместо path-фильтра, а ПЕРЕД
    // блок-листом доменов, потому что youtube.com из блок-листа не вынимается.
    //
    // Что НЕ снимается никогда: расписание блокировки компьютера. Вне окна
    // расписания экран закрыт в любой день — это критфикс v0.17.2, и повторён он
    // здесь сознательно: «открыть ютуб» не должно означать «сидеть до ночи».
    // ═════════════════════════════════════════════════════════════════════════

    function isYoutubeHost() {
        return hostMatches((CONFIG.youtube && CONFIG.youtube.hosts) || []);
    }

    // Спутник — только ТОЧНОЕ совпадение, без поддоменов. Здесь и живёт вся
    // разница между «открыт поиск» и «открыт весь гугл вместе с магазином
    // расширений»: hostMatches сюда звать нельзя.
    function isYoutubeCompanionHost() {
        const hosts = (CONFIG.youtube && CONFIG.youtube.companionHosts) || [];
        return hosts.some((host) => HOST === host);
    }

    function isYoutubeGrantHost() {
        return isYoutubeHost() || isYoutubeCompanionHost();
    }

    // Выполнена ли дневная норма задач. Отдельная функция, а не переиспользование
    // isLichessFullUnlockEarnedByTasks: та сперва смотрит на тумблер награды
    // lichess, и выключенная награда молча закрывала бы ютуб навсегда.
    function isDailyTaskTargetReached(now = new Date()) {
        const target = getDailyTarget(now);
        if (!(target > 0)) return false;
        return readNumber(trackerKeys(formatDateKey(now)).racerSolved, 0) >= target;
    }

    // Окна, внутри которых разрешение действует:
    //   персональные часы на дату → они;
    //   режим 'always'            → null (весь календарный день);
    //   режим 'schedule'          → окна расписания дня.
    function getYoutubeUnlockWindowsForDate(date = new Date()) {
        const custom = ((CONFIG.youtube && CONFIG.youtube.unlockWindows) || {})[formatDateKey(date)];
        if (Array.isArray(custom) && custom.length) {
            return custom
                .map(([from, to]) => ({ start: parseTimeString(from), end: parseTimeString(to) }))
                .filter((windowItem) => windowItem.start < windowItem.end)
        ...
        if (CONFIG.youtube && CONFIG.youtube.unlockMode === 'always') return null;
        return getUnlockedWindowsForDate(date);
    }

    function getActiveYoutubeUnlockWindow(now = new Date()) {
        const windows = getYoutubeUnlockWindowsForDate(now);
        if (windows === null) return { start: 0, end: 24 * 60 };
        const currentMinutes = getCurrentMinutes(now);
        return windows.find((windowItem) => currentMinutes >= windowItem.start && currentMinutes < windowItem.end) || null;
    }

    // Главная проверка: открыт ли ютуб ИМЕННО СЕЙЧАС и ИМЕННО ЗДЕСЬ.
    // Проверка хоста стоит первой и не случайно: это единственное, что не даёт
    // разрешению протечь на соседний домен, если в файле с сервера окажется мусор.
    // Действует ли разрешение ПРЯМО СЕЙЧАС — без оглядки на то, где мы находимся.
    // Нужно на чужом хосте: блок-экран обязан знать, что ютуб сегодня открыт,
    // чтобы предложить туда ссылку.
    function isYoutubeGrantActiveNow(now = new Date()) {
        const cfg = CONFIG.youtube;
        if (!cfg) return false;
        if (!Array.isArray(cfg.unlockDates) || !cfg.unlockDates.includes(formatDateKey(now))) return false;
        if (cfg.afterTaskTarget && !isDailyTaskTargetReached(now)) return false;
        return !!getActiveYoutubeUnlockWindow(now);
    }

    function isYoutubeUnlockedNow(now = new Date()) {
        if (!isYoutubeGrantHost()) return false;
        return isYoutubeGrantActiveNow(now);
    }

    // v0.24.2: ссылки на блок-экране. Ребёнок не набирает адрес целиком — он
    // пишет слово в адресной строке, оно уходит в поиск, и блок-экран выпадает
    // на google.com. Дальше он видит три ссылки на шахматы и делает вывод, что
    // ютуб закрыт, хотя тот открыт и ждёт. Пока разрешение действует, ссылка на
    // ютуб стоит ПЕРВОЙ: блок-экран должен говорить, куда можно, а не только то,
    // что сюда нельзя.
    function getQuickLinks() {
        const links = (CONFIG.urlBlocker.quickLinks || []).slice();
        if (isYoutubeGrantActiveNow()) {
            links.unshift(['YouTube — сегодня открыт', 'https://www.youtube.com/']);
        }
        return links;
    }

    // Время разрешения кончилось на уже открытой вкладке — закрываем её блок-экраном,
    // не дожидаясь навигации. Тикает вместе с timeBlocker (раз в минуту и по poll).
    // Без этого окно «до 17:00» закрывалось бы только при переходе по ссылке.
    function refreshYoutubeUnlockState() {
        if (!isYoutubeGrantHost()) return false;
        const active = isYoutubeUnlockedNow();
        if (active === RUNTIME.youtube.active) return active;
        RUNTIME.youtube.active = active;
        log('youtube unlock state changed', { active });
        if (!active) initUrlBlocker();
        return active;
    }
    ...
    function armCurtain(root) {
        try {
            ...
            if (el && el.setAttribute) {
                el.setAttribute('data-ucc-armed', '1');
                return;
            }
            // v0.24.1: на очень раннем внедрении <html> ещё не создан, и попытка
            // снять шторку молча пропадала — страница оставалась под «Загрузка…».
            // Раньше это не всплывало: на разрешённых сайтах скрипт доезжает
            // сюда позже, а все остальные закрывал блок-экран, который пишет
            // атрибут прямо в свой <html>.
            onReady(() => {
                const late = document.documentElement;
                if (late && late.setAttribute) late.setAttribute('data-ucc-armed', '1');
            });
        } catch (error) {
    ...
    // v0.24.4: разметка блок-экрана собирается ОДИН раз и подаётся двумя путями.
    // Стиль вынесен отдельно, потому что нужен обоим.
    const BLOCK_SCREEN_CSS = `
    body {
      margin: 0;
      min-height: 100vh;
      display: flex;
...
`;
    ...
    // Запасной путь показа блок-экрана — через DOM, без единого HTML-приёмника.
    // Нужен на сайтах с Trusted Types (youtube.com): там и document.write, и
    // innerHTML бросают TypeError, блок-экран не появлялся вовсе, а страница
    // оставалась под шторкой Stylus с вечной «Загрузка…».
    function renderBlockScreenViaDom(title, heading, message, links) {
        try { document.title = title; } catch (error) { /* не критично */ }

        const style = document.createElement('style');
        style.textContent = BLOCK_SCREEN_CSS;

        const card = document.createElement('div');
        card.className = 'ucc-blocker';
        const headingEl = document.createElement('h1');
        headingEl.textContent = heading;
        const messageEl = document.createElement('p');
        messageEl.textContent = message;
        const linkBox = document.createElement('div');
        linkBox.className = 'ucc-links';
        ...
        card.append(headingEl, messageEl, linkBox);

        let stopped = false;
        const apply = () => {
            const root = document.documentElement;
            // До появления <html> делать нечего. И, главное, нельзя звать
            // window.stop(): на самом раннем внедрении он обрывает саму
            // навигацию, и вкладка уходит не в блок-экран, а в ошибку сети.
            if (!root) return;
            if (!stopped) {
                stopped = true;
                try { window.stop(); } catch (error) { /* страница уже догрузилась */ }
            }
            ...
            if (!card.isConnected || card.parentNode !== body) {
        ...
        apply();
        onReady(apply);
        // Документ мы не заменили целиком, поэтому скрипты сайта живы и SPA может
        // отрисовать свой контент поверх. Держим экран на месте: проверка дешёвая,
        // и без неё блокировка на таком сайте живёт секунду.
        const guard = window.setInterval(apply, 400);
        window.setTimeout(() => window.clearInterval(guard), 15000);
    }

    // Работают ли вообще HTML-приёмники на этой странице. Проверяем ДО
    // document.open: он сносит текущий документ первым же вызовом, и если
    // document.write следом бросит, ловить будет уже нечего — страница уйдёт
    // в никуда. Проба на отсоединённом узле безвредна и стоит микросекунды.
    let htmlSinksUsable = null;
    function canUseHtmlSinks() {
        if (htmlSinksUsable !== null) return htmlSinksUsable;
        try {
            document.createElement('div').innerHTML = '<i></i>';
            htmlSinksUsable = true;
        } catch (error) {
            log('HTML-приёмники закрыты политикой страницы (Trusted Types)', error);
            htmlSinksUsable = false;
        }
        return htmlSinksUsable;
    }

    function replaceDocument(title, heading, message, links = []) {
        if (!canUseHtmlSinks()) {
            sanitizeBlockedUrl();
            renderBlockScreenViaDom(title, heading, message, links);
            return;
        }
...
<body>
  <div class="ucc-blocker">
    ...
    <div class="ucc-links">${linkHtml}</div>
            ...
            // document.write — HTML-приёмник. Под Trusted Types он бросает, и
            // раньше вместе с ним умирала вся блокировка: ни экрана, ни объяснения.
            log('replaceDocument: document.write недоступен, собираю через DOM', error);
            renderBlockScreenViaDom(title, heading, message, links);
            return;
        }
    ...
    // v0.19: производные значения. Часть конфига собирается ИЗ ДРУГОЙ части один раз,
    // при сборке LOCAL_CONFIG: lichess.minBaseMinutes — это chessCom.minBaseTimeSeconds,
    // пересчитанные в минуты. После удалённой правки такую связь надо восстанавливать
    // руками, иначе минимальный контроль времени поменяется только на chess.com, а на
    // lichess останется прежним — и половина ограничения тихо разъедется с другой.
    function syncDerivedConfig() {
        const seconds = Number(CONFIG.chessCom && CONFIG.chessCom.minBaseTimeSeconds);
        if (Number.isFinite(seconds) && seconds > 0) {
            CONFIG.lichess.minBaseMinutes = Math.floor(seconds / 60);
        }
    }

    // Накладывает удалённый конфиг поверх локального.
    // Порядок важен: СНАЧАЛА все управляемые поля откатываются к локальным значениям,
    ...
    // v0.21: номер загрузки страницы. Одна навигация = один номер, поэтому в
    // журнале видно, что «развернуло на гонку» и «страница гонки открылась» —
    // это одна история, а не два независимых события.
    const TELEMETRY_NAV = Math.random().toString(36).slice(2, 8);

    // v0.21: состояние, в котором принято решение. Без него вердикт бесполезен:
    // «развернуло на гонку» ничего не говорит, пока неизвестно, сколько задач
    // решено и какая сегодня норма. Читаем только из хранилища, ничего не считая
    // заново, чтобы съём не стоил дороже самого решения.
    function telemetryContext() {
        try {
            const now = new Date();
            const keys = trackerKeys(formatDateKey(now));
            const solved = readNumber(keys.racerSolved, 0);
            const target = getDailyTarget(now);
            const windows = getUnlockedWindowsForDate(now);
            const currentMinutes = getCurrentMinutes(now);
            const active = windows.find((w) => currentMinutes >= w.start && currentMinutes < w.end);
            return {
                solved,
                target,
                norm: solved >= target ? 'выполнена' : 'не выполнена',
                window: active
                    ? `${minutesToTimeString(active.start)}-${minutesToTimeString(active.end)}`
                    : 'вне окна',
                unlocked: isLichessFullyUnlockedNow() ? 'да' : 'нет'
            };
        } catch (error) {
            return {};
        }
    ...
    function telemetryPush(event, fields = {}) {
        try {
            ...
            const row = Object.assign({
                ...
                // Откуда пришли. Единственный способ понять, какую именно ссылку
                // или кнопку нажимали перед тем, как их развернуло.
                from: (document.referrer || '').slice(0, 120),
            ...
            // v0.21: отказ, разворот и падение отправляем НЕМЕДЛЕННО. Эти ветки
            // кончаются блок-экраном или редиректом, страница уходит из-под ног,
            // и по обычному расписанию запись дождалась бы следующей открытой
            // страницы — то есть самое нужное приходило бы последним или никогда.
            // Запрос живёт в контексте расширения и навигацию переживает.
            if (row.verdict === 'blocked' || row.verdict === 'redirect' ||
                row.verdict === 'exception' || row.verdict === 'rejection') {
                telemetryFlush();
            }
        } catch (error) {
    ...
    // Версию берём у самого расширения, а не зашиваем строкой: зашитая рано или
    // поздно разъедется с реальной, и журнал начнёт врать именно тогда, когда по
    // нему будут разбирать, обновился скрипт или нет.
    function scriptVersion() {
        try {
            return (typeof GM_info === 'object' && GM_info && GM_info.script && GM_info.script.version) || '?';
        } catch (error) {
            return '?';
        }
    }
    ...
    function telemetryFlush() {
        try {
            ...
            GM_xmlhttpRequest({
                ...
                data: JSON.stringify({ v: scriptVersion(), events: batch }),
                timeout: 10000,
                onload: (response) => {
    ...
    // v0.24.1: ловушка ошибок вынесена из initTelemetry и поднимается ПЕРВЫМ
    // делом. Раньше слушатели ставились последней строкой инициализации, и
    // падение внутри самой инициализации не попадало в журнал ВООБЩЕ — то есть
    // ровно та беда, ради которой журнал заводили, оставалась невидимой.
    // Симптом 20.09: на youtube.com страница навсегда осталась под шторкой
    // Stylus («Загрузка…»), а в журнале за этот заход — одна строка prime.
    function initErrorCapture() {
        if (!CONFIG.telemetry || !CONFIG.telemetry.enabled) return;

        // v0.21: лимит на загрузку нужен, чтобы сломанный сайт не забил журнал
        // своими ошибками: здесь ловятся все, отличить чужие от своих нельзя.
        let errorsSent = 0;
        window.addEventListener('error', (e) => {
            ...
            telemetryPush('error', {
                verdict: 'exception',
                reason: String((e && e.message) || 'ошибка').slice(0, 200),
                detail: { src: String((e && e.filename) || '').slice(0, 120), line: (e && e.lineno) || 0 }
            });
        });
        window.addEventListener('unhandledrejection', (e) => {
            ...
            telemetryPush('error', {
                verdict: 'rejection',
                reason: String((e && e.reason) || 'отказ').slice(0, 200)
            });
        });

    }

    // Одна строка в журнал на упавший шаг инициализации. Отдельно от слушателя
    // window.error: тот ловит только то, что дошло наверх, а шаг под try/catch
    // до него не доходит — и молчание опять выглядело бы как «всё хорошо».
    function reportInitFailure(step, error) {
        log(`инициализация: шаг «${step}» упал`, error);
        telemetryPush('error', {
            verdict: 'exception',
            reason: `${step}: ${String((error && error.message) || error).slice(0, 160)}`
        });
    }

    // Есть ли у текущего хоста правила отправки. Нужно, чтобы решить, что делать
    // с упавшим предохранителем: охранять нечего — идём дальше, есть что —
    // остаёмся закрытыми.
    function hostHasSendGuardRules() {
        try {
            return !!sendGuardRulesForHost(compileSendGuardRules(), HOST);
        } catch (error) {
            return true; // не смогли выяснить — считаем, что правила есть
        }
    }
    ...
    // v0.22: кэш удалённых настроек применяется ДО того, как модули начнут решать.
    // Это не косметика порядка, а условие работоспособности: раньше продление окна
    // и снятие нормы приезжали уже ПОСЛЕ того, как страницу развернуло, а в ветке
    // редиректа трекера — не приезжали никогда. Сеть по-прежнему опрашивается
    // позже и применяется на лету; здесь только кэш, то есть последнее, что
    // сервер уже успел сказать.
    //
    // Почему это по-прежнему безопасно:
    //   • у кэша есть срок годности, протухший стирается и в дело не идёт;
    //   • пустой кэш означает работу по локальным настройкам, они самые строгие;
    //   • подменить кэш можно только имея доступ к машине ребёнка — а имея его,
    //     проще выключить само расширение.
    function primeConfigFromCache() {
        if (!CONFIG.remoteConfig.enabled) return false;
        const cached = readCachedRemoteConfig();
        if (!cached) {
            telemetryPush('prime', { verdict: 'local', reason: 'кэша нет, работаем по заводским' });
            return false;
        }
        applyRemoteConfig(cached);
        RUNTIME.remoteConfig.lastAppliedSignature = getRemoteConfigSignature(cached);
        log('config primed from cache');
        telemetryPush('prime', { verdict: 'cache', reason: 'решения по кэшу настроек' });
        return true;
    }

    // Чтение кэша со сроком годности. Вынесено отдельно, чтобы одну и ту же
    // проверку не пришлось повторять в двух местах и разъехаться на третьем.
    function readCachedRemoteConfig() {
        const cached = readValue(CONFIG.storage.remoteConfigCacheKey, null);
        if (!cached || typeof cached !== 'object') return null;

        // v0.19: срок годности кэша. Без этой проверки последний удачно скачанный файл
        // применялся бы вечно — достаточно было один раз получить щедрое расписание и
        // потом остаться без сети. Просроченный кэш стираем целиком вместе с метаданными.
        const meta = readValue(CONFIG.storage.remoteConfigMetaKey, null);
        const updatedAt = meta ? Number(meta.updatedAt) : NaN;
        const maxAge = Number(CONFIG.remoteConfig.cacheMaxAgeMs);
        if (Number.isFinite(maxAge) && maxAge > 0) {
            const expired = !Number.isFinite(updatedAt) || (Date.now() - updatedAt) > maxAge;
            if (expired) {
                log('remote config cache expired, falling back to local settings');
                deleteValue(CONFIG.storage.remoteConfigCacheKey);
                deleteValue(CONFIG.storage.remoteConfigMetaKey);
                return null;
            }
        }
    ...
    function initRemoteConfig() {
        // Кэш уже применён в primeConfigFromCache() до старта модулей — повторять
        // нечего. Здесь только сеть: свежий файл и дальнейший опрос по таймеру.
        if (!CONFIG.remoteConfig.enabled) return;
        fetchRemoteConfig();
    ...
    // v0.24.1. Скрипт работает в 'use strict', и `page.fetch = ...` — голое
    // присваивание. На сайте, где интринсик объявлен неперезаписываемым (frozen
    // intrinsics, Trusted Types, чужое расширение), такое присваивание не «просто
    // не срабатывает», а бросает TypeError и уносит с собой ВСЮ инициализацию —
    // вместе со снятием шторки. Страница остаётся под «Загрузка…» навсегда.
    // Раньше этого никто не видел: любой сайт вне двух разрешённых закрывался
    // блок-экраном ДО того, как дело доходило до реальной работы предохранителя.
    // patchSlot ставит перехват там, где можно, и молча отступает там, где нельзя,
    // — решение по странице от этого зависеть не должно.
    function patchSlot(owner, key, make, label) {
        try {
            const native = owner && owner[key];
            if (typeof native !== 'function') return false;
            const descriptor = Object.getOwnPropertyDescriptor(owner, key);
            if (descriptor && !descriptor.writable && !descriptor.set) {
                log(`sendGuard: ${label} только для чтения, перехват пропущен`);
                return false;
            }
            owner[key] = make(native);
            return true;
        } catch (error) {
            log(`sendGuard: не удалось перехватить ${label}`, error);
            return false;
        }
    }
    ...
    function installSendGuard() {
        ...
        patchSlot(page, 'fetch', (nativeFetch) => function guardedFetch(input, init) {
                try {
        ...
        if (xhrProto) {
            patchSlot(xhrProto, 'open', (nativeOpen) => function guardedXhrOpen(method, url) {
                try {
            ...
            patchSlot(xhrProto, 'send', (nativeSend) => function guardedXhrSend() {
        ...
        if (typeof nativeWs === 'function' && nativeWs.prototype && ((CONFIG.sendGuard.denyWsPayload) || []).length) {
            patchSlot(nativeWs.prototype, 'send', (nativeWsSend) => function guardedWsSend(data) {
                try {
                    syncGuardRules();
        ...
        if (pageNavigator) {
            patchSlot(pageNavigator, 'sendBeacon', (nativeBeacon) => function guardedBeacon(url) {
                try {
        ...
        if (formProto) {
            patchSlot(formProto, 'submit', (nativeFormSubmit) => function guardedFormSubmit() {
                try {
    ...
    function initUrlBlocker() {
        ...
        // v0.24: разовое разрешение ютуба. Стоит ВЫШЕ блок-листа сознательно:
        // youtube.com из blockedHosts не вынимается, разрешение лишь перешагивает
        // его на сегодня и только на своих хостах (см. isYoutubeUnlockedNow).
        if (isYoutubeUnlockedNow()) {
            telemetryPush('urlblock', {
                verdict: 'allowed',
                reason: isYoutubeHost() ? 'ютуб открыт на сегодня' : 'спутник ютуба открыт на сегодня'
            });
            return false;
        }
    ...
    function ensureTimeOverlay() {
        ...
        // v0.24.4: сборка через DOM, а не innerHTML. Этот узел создаётся на КАЖДОЙ
        // странице, в том числе разрешённой: applyState зовёт hideBlockedOverlay,
        // чтобы спрятать экран, и создаёт его по дороге. На youtube.com с Trusted
        // Types присваивание innerHTML бросало TypeError, main() умирал до снятия
        // шторки, и вместо ютуба ребёнок видел вечную «Загрузка…» (журнал 21.09).
        const overlayCard = document.createElement('div');
        overlayCard.className = 'ucc-time-blocker-card';
        const overlayTitle = document.createElement('h1');
        overlayTitle.id = 'ucc-time-blocker-title';
        overlayTitle.textContent = 'Время закончилось';
        const overlayMessage = document.createElement('p');
        overlayMessage.id = 'ucc-time-blocker-message';
        overlayCard.append(overlayTitle, overlayMessage);
    ...
    function initTimeBlocker() {
        ...
        const applyState = () => {
            ...
            // v0.24: и окно разрешённого ютуба — им же.
            refreshYoutubeUnlockState();
    ...
    function createProgressWindow() {
        ...
        // v0.24.4: через DOM, без innerHTML — см. ensureTimeOverlay.
        [
            ['tasks', '🎯 ', false],
            ['bullet', '⚡ ', true],
            ['unlock', '🔓 ', true]
        ].forEach(([role, icon, hidden]) => {
            const row = document.createElement('div');
            row.className = 'ucc-progress-row';
            row.dataset.role = `${role}-line`;
            if (hidden) {
                row.style.marginTop = '6px';
                row.style.display = 'none';
            }
            const text = document.createElement('span');
            text.dataset.role = `${role}-text`;
            text.textContent = '…';
            row.append(document.createTextNode(icon), text);
    ...
    function updateProgressWindow() {
        ...
        if (tasksLine && tasksText) {
            if (tasksDone) {
                ...
                tasksLine.firstChild.nodeValue = '🎯 ';
                // v0.24.4: без innerHTML — см. ensureTimeOverlay.
                const solvedEl = document.createElement('strong');
                solvedEl.textContent = String(state.solved);
                const leftEl = document.createElement('span');
                leftEl.style.color = '#ffe17e';
                leftEl.textContent = `−${state.remaining}`;
                tasksText.replaceChildren(
                    solvedEl,
                    document.createTextNode(` / ${state.target} задач · `),
                    leftEl
                );
            }
        }
    ...
    function initTracker() {
        ...
        const enforceGate = () => {
            ...
            // v0.24: в разрешённый день ютуб не разворачивает на задачи. Если
            // норму всё же хочется требовать вперёд — youtube.afterTaskTarget,
            // тогда isYoutubeUnlockedNow() сам вернёт false до её выполнения.
            if (!isLichessFullyUnlockedNow() && !isYoutubeUnlockedNow() &&
                !currentState.unlockGranted && !currentIsSourcePage) {
        ...
        if (!isLichessFullyUnlockedNow() && !isYoutubeUnlockedNow() &&
            !state.unlockGranted && !isSourcePage) {
            const target = getTrainingRedirectTarget();
            if (HREF !== target) {
                telemetryPush('gate', {
                    verdict: 'redirect',
    ...
    // v0.22: ПЕРВЫМ делом накладываем кэш удалённых настроек. Все модули ниже
    // принимают решения уже по нему, а не по заводским значениям. Пересборку
    // модулей здесь не зовём сознательно: их ещё нет, да и незачем — они вот-вот
    // поднимутся и прочитают готовый CONFIG.
    // Сеть опрашивается в конце, см. initRemoteConfig() после armCurtain().
    primeConfigFromCache();

    // v0.24.1: ловушка ошибок — ДО всего остального. Иначе падение любого шага
    // ниже не попадёт в журнал, и разбираться придётся по фразе «не работает».
    initErrorCapture();
    ...
    //
    // v0.24.1: и он же — место, где 20.09 сломался ютуб. Скрипт работает в
    // 'use strict', а перехват ставился голым присваиванием `page.fetch = ...`.
    // На сайте, где интринсик неперезаписываемый, это не «не сработало», а
    // TypeError, уносивший ВСЮ инициализацию — включая снятие шторки. Внутри
    // installSendGuard каждое присваивание теперь идёт через patchSlot, а здесь
    // ловим остаток: если у хоста правил отправки нет, охранять нечего и прятать
    // страницу не за что; если есть (chess.com, lichess) — это отказ последней
    // линии, и страница остаётся закрытой, как и раньше.
    try {
        installSendGuard();
    } catch (error) {
        reportInitFailure('предохранитель отправки', error);
        if (hostHasSendGuardRules()) throw error;
    }
    ...
    // v0.24.1: косметические фильтры под try/catch. Они прячут ссылки на
    // закрытые разделы — полезно, но это не запрет, а оформление. Падение
    // любого из них не должно стоить ребёнку страницы: раньше оно уносило
    // armCurtain ниже по коду, и вместо сайта человек получал вечную
    // «Загрузка…». Порядок сохранён: фильтры отрабатывают ДО снятия шторки,
    // поэтому неотфильтрованный lichess по-прежнему не мелькает.
    try {
        ...
        reportInitFailure('фильтры страницы', error);
    }
    ...
    // v0.19: удалённый конфиг — ПОСЛЕДНИМ шагом, уже после того как поднялись все модули
    // и снята шторка. Порядок выбран сознательно, он же и есть fail-closed: на холодном
    // старте первые доли секунды действуют локальные настройки скрипта (самые строгие),
