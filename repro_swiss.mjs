...
// ── 4. Ссылка на команду со страницы швейцарки ведёт на живую страницу ──
// Ожидание перевёрнуто по сравнению с v0.15, и это не послабление, а догон за
// v0.23: швейцарки проводят команды, и ребёнок не мог вступить — упирался в
// блок-экран. Страница команды /team/<имя> открыта точечным правилом, каталог
// /team и подразделы (форум, рассылка, участники) закрыты как прежде.
// Прибор до сих пор требовал прятать эту ссылку и горел красным на решении,
// которое приняли сознательно.
{
    ...
    check('ссылка на команду со страницы швейцарки видна', s.total > 0 && s.visible === s.total, `${s.visible}/${s.total}`);
    await p.close();
}
