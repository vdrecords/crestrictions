...
**Версия:** см. `@version` в `cc.user.js` (актуально 0.15.0).
...
- `/swiss/new/<team>` (создание собственной швейцарки — `/swiss` и `/swiss/<id>` открыты с 0.15.0)
...
2. Зайти на chess.com и lichess.org, проверить:
   ...
   - `/simul`, `/inbox`, `/team`, `/forum`, `/clubs`, `/messages`, `/tv`, `/streamer` → «Раздел не разрешён»
   - `/swiss` (расписание швейцарок) — открывается, Пуля/UltraBullet/варианты в списке скрыты; `/swiss/new/<team>` → «Раздел не разрешён»
...
- **0.15.0** (2026-07-30) — швейцарские турниры lichess разрешены. Раздел был закрыт целиком с 0.4.0 (2026-05-09, «идёт долго — потеря времени»). Теперь открыты `/swiss` (расписание) и `/swiss/<id>` (участие), в block остаётся только `/swiss/new/<team>` — создание собственного турнира, симметрично `/tournament/new` у Арены. Правила отбора те же, что у Арены, но разметка другая (не карточки `.tsht`, а строки таблицы `table.swisses`, где контроль и тип лежат в одной подписи `.setup`: «30+0 • Классика • Рейтинговый»). Поэтому добавлены: (1) `filterSwissRows` — фильтр строк расписания: UltraBullet (¼+0/½+0) и варианты (Atomic/Crazyhouse/960 — на швейцарках вариант подставляется ВМЕСТО типа: «3+0 • Crazyhouse») получают класс `.ucc-blocked-swiss` и скрыты всегда; Пуля 1+0/2+1 — класс `.ucc-blocked-swiss-bullet`, снимается при открытом Bullet-окне через `body.ucc-bullet-window-open`. Проверка варианта идёт по БАЗОВОМУ списку типов, а не через `textHasAllowedType`, иначе «1+0 • Crazyhouse» при открытом Bullet-окне прошёл бы как обычная Пуля. (2) Маркер `.swiss__meta` в `hasGameContext` + чтение контроля из первого `<p>` в `detectGameTypeText` — на самой странице турнира при несоответствии прячутся кнопка «Участвовать» и доска. Ссылки на `/team` в карточках швейцарок по-прежнему скрыты (`initAutoHideBlockedPaths`), чат зрителей `.mchat` — тоже. Замер headless-Chrome 2026-07-30 на реальных страницах lichess (`repro_swiss.mjs`, фикстуры `fx_swiss/`): 16/16 — расписание открывается, 8 строк Рапид/Блиц/Классика видны, Пуля и ½+0 скрыты, Crazyhouse 5+0 и 1+0 скрыты навсегда, страницы 30+0 и 5+0 играбельны, 1+0 и ½+0 — с убранной доской и кнопкой участия, `/swiss/new/<team>` даёт блок-экран, `/training` и `/tournament` не задеты. На 0.14.0 та же проба падает (0/1, «Раздел заблокирован»)
- **0.12.4** (2026-05-09) — **критфикс:** доска не отображалась на странице Bullet-партии при открытом Bullet-окне. Корень: `textHasAllowedType` проверяет gameTypeText против `lichessCfg.allowedGameTypes = ['Блиц','Рапид','Классика','Blitz','Rapid','Classical','SuperBlitz']`. На странице Bullet-партии Lichess пишет «1+0 • Пуля • Xм» — `Пуля`/`Bullet` в whitelist НЕТ → `textHasAllowedType` возвращал false → applyRules не делал early-return → срабатывал последний этап со скрытием `.cg-wrap` / `.main-board` через `safeHide(boardSelectors)`. Симметрия с уже работающим `getEffectiveMinMinutes()` (он динамически снижает minMinutes до 1 при открытом Bullet) — нужна была такая же динамика и для game-type whitelist. Фикс: `textHasAllowedType` теперь добавляет `'Пуля'`/`'Bullet'` в типы при `bullet.isOpen`. Прецедент 2026-05-09 19:00+: Vladimir запустил Bullet-партию в forceOpen окно — попал в игру, но доска была скрыта. Дополнительный side-effect: турнирные карточки/страницы с непарсимым временем но содержащим 'Пуля' теперь тоже признаются allowed при открытом окне (соответствует общей логике)
- **0.12.3** (2026-05-09) — `BULLET_REWARD_FORCE_OPEN_DATES` — особые дни (день рождения, праздник, болезнь и т.п.) когда Bullet-окно гарантировано открыто на полный час (60 мин в конце расписания) НЕЗАВИСИМО от количества решённых задач. Применение: добавить дату `'YYYY-MM-DD'` в массив. `disabledDates` имеет приоритет (если дата в обоих списках — Bullet закрыт). Если ребёнок и так заработал 60 мин по задачам — окно остаётся 60 мин (не суммируется). UI помечает особый день префиксом `🎁 Особый день · 19:00–20:00 (60 мин)`. На сегодня (2026-05-09) поставлена `['2026-05-09']` чтобы первый раз сработало в этот же вечер. Архитектурно поле было в CONFIG.bulletReward с v0.12.0 как заглушка, но логика `computeBulletReward` его игнорировала — v0.12.3 это починила
- **0.12.2** (2026-05-09) — компактный 2-строчный layout окна прогресса (откат кнопок свернуть/закрыть из v0.12.1). Из 8 строк (заголовок + 3 ряда задач + разделитель + Bullet header + 2 строки текста) → в 2 inline-строки: `🎯 N/M задач · −R` (либо `✅ Цель выполнена (N)`) + `⚡ <inline-статус Bullet>`. 4 состояния Bullet объединены в одну фразу: `+400 задач → 10 мин в конце дня` / `19:40–20:00 (20 мин) · +80 → 30 мин` / `Активен · 4:23 до 20:00` / `19:00–20:00 (60 мин — максимум)`. Кнопки «−»/«×» удалены — окно теперь достаточно маленькое чтобы не мешать. RUNTIME-флаги `progressClosedSession` и GM-key `ucc_progress_collapsed` удалены за ненадобностью. Уменьшение габаритов: padding 14×16 → 10×14, font 14/1.45 → 13/1.4, max-width убран как лишний (содержимое само регулирует ширину)
- **0.12.1** (2026-05-09) — [откачен в 0.12.2] кнопки управления окном прогресса. В правом верхнем углу окна `−` (свернуть) и `×` (закрыть). Свёрнутое состояние сохраняется в GM-storage (`ucc_progress_collapsed`) — переживает перезагрузку страницы и применяется на всех табах chess.com / lichess.org. При свёрнутости видно только заголовок `Прогресс задач` / `Цель выполнена ✅` + кнопка `+` для разворачивания, тело окна не обновляется до восстановления. Кнопка «×» закрывает окно ДО следующей перезагрузки страницы (`RUNTIME.tracker.progressClosedSession = true` runtime-only) — ребёнок не может «выключить» окно навсегда без помощи родителя. Bullet-фильтр (`body.ucc-bullet-window-open` синхронизация) работает независимо от collapsed/closed-состояний — окно скрыто, а Bullet-доступ соответствует реальному статусу
- **0.12.0** (2026-05-09) — Bullet-награда (BULLET_REWARD блок в USER_SETTINGS). Когда ребёнок решает ≥400 задач Lichess Racer — в конце последнего интервала расписания дня автоматически открывается окно Bullet (1+0). Динамический рост: `400=10мин, +10мин на каждые +100 задач, cap 60мин/день`. Никаких кнопок — окно открывается само в `scheduleEnd − earnedMinutes`, закрывается на `scheduleEnd` (= конец сессии, активная партия не прерывается). Применимо к ЛЮБОМУ дню недели (привязки только к выходным НЕТ): в будни цель 100 → порог 400 физически недостижим, в выходные 1000 → доступно до 4-6 окон расширения. Прозрачный UI в окне прогресса (4 состояния): «недоступен / до открытия N задач», «доступен с HH:MM до HH:MM», «активен — осталось M:SS», «cap 60 мин — конец сессии HH:MM». Master toggle `BULLET_REWARD_ENABLED = false` отключает всю логику одной строкой. CSS-механизм через body class `ucc-bullet-window-open` (overrides default-hide на `.tsht-short` / `.ucc-blocked-tour-bullet` / `bulletSection chess.com`). UltraBullet (¼+0, ½+0) и варианты (Atomic/Crazyhouse/960) — НЕ открываются никогда. Для chess.com `/play/online` секция Пуля и Bullet турниры тоже разблокируются окном; alert при попытке выбрать ниже минимума уведомляет «Bullet-окно открыто — разрешён 1+0»
- **0.11.2** (2026-05-09) — разовая цель: `TASK_TARGETS_SPECIAL['2026-05-09'] = 50` (вместо обычной субботней 1000)
